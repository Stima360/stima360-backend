...
GITHUB_PDF_BASE_URL = os.getenv(
    "GITHUB_PDF_BASE_URL",
    "https://raw.githubusercontent.com/Stima360/stima360-pdf/main"  # puoi cambiare se metti una sottocartella
)
...
def _logo_flowable(logo_path: str, target_h_cm: float = 2.0):
    ...
    """
    from reportlab.platypus import Spacer as RLSpacer  # evita ombra sul nome

    if not logo_path or not os.path.exists(logo_path):
        return RLSpacer(0, target_h_cm * cm)
    try:
...
def genera_pdf_stima(dati: dict, nome_file: str = "stima360.pdf"):
    ...
    - RITORNA un URL GitHub del tipo:
      GITHUB_PDF_BASE_URL + '/' + nome_file
    """
    ...
    # 🔗 LINK CHE USERAI NEL SITO / EMAIL / WHATSAPP → GITHUB
    pdf_web_path = f"{GITHUB_PDF_BASE_URL.rstrip('/')}/{nome_file}"
    ...
    try:
        if prezzo_base:
            ratio = (
                eur_mq_finale / float(prezzo_base)
                if float(prezzo_base)
                else 1.0
            )
    ...
    # micro grafico comparabili
    safe_vals = _parse_comparabili(dati.get("comparabili"))
    d = Drawing(400, 130)
    bc = VerticalBarChart()
    bc.x = 30
    bc.y = 20
    ...
    # ⬅️ Ritorno URL GitHub, usato come link in email/WhatsApp/frontend
    return pdf_web_path
