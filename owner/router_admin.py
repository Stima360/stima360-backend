...
from .schemas import (
    AccessCreate, AccountCreate, FeedbackStatus, PublicationCreate,
    PublicationUpdate, RevokeRequest, SharedDocumentCreate,
    SharedDocumentSupersede, SharedDocumentUpdate, TokenCreate,
    VisitFeedbackCreate, VisitFeedbackSupersede, VisitFeedbackUpdate,
)
...
def token(i: int, p: TokenCreate):
    ...
    return {"token_id": row["id"], "expires_at": row["expires_at"], "token": raw, "one_time_display": True}
...
def feedback_status(i: int, p: FeedbackStatus): return x(r.update_feedback_status, i, p.model_dump(exclude_unset=True))

@router.get("/documents")
def documents(): return {"items": x(r.list_shared_documents)}

@router.post("/documents", status_code=201)
def document_create(p: SharedDocumentCreate): return x(r.create_shared_document, p.model_dump())

@router.patch("/documents/{i}")
def document_update(i: int, p: SharedDocumentUpdate): return x(r.update_shared_document, i, p.model_dump(exclude_unset=True))

@router.post("/documents/{i}/publish")
def document_publish(i: int): return x(r.publish_shared_document, i)

@router.post("/documents/{i}/revoke")
def document_revoke(i: int, p: RevokeRequest): return x(r.revoke_shared_document, i, p.actor)

@router.post("/documents/{i}/archive")
def document_archive(i: int): return x(r.archive_shared_document, i)

@router.post("/documents/{i}/supersede", status_code=201)
def document_supersede(i: int, p: SharedDocumentSupersede): return x(r.supersede_shared_document, i, p.model_dump())

@router.get("/visit-feedback")
def visit_feedback(): return {"items": x(r.list_visit_feedback_publications)}

@router.post("/visit-feedback", status_code=201)
def visit_feedback_create(p: VisitFeedbackCreate): return x(r.create_visit_feedback_publication, p.model_dump())

@router.patch("/visit-feedback/{i}")
def visit_feedback_update(i: int, p: VisitFeedbackUpdate): return x(r.update_visit_feedback_publication, i, p.model_dump(exclude_unset=True))

@router.post("/visit-feedback/{i}/publish")
def visit_feedback_publish(i: int): return x(r.publish_visit_feedback, i)

@router.post("/visit-feedback/{i}/archive")
def visit_feedback_archive(i: int): return x(r.archive_visit_feedback, i)

@router.post("/visit-feedback/{i}/supersede", status_code=201)
def visit_feedback_supersede(i: int, p: VisitFeedbackSupersede): return x(r.supersede_visit_feedback, i, p.model_dump())
