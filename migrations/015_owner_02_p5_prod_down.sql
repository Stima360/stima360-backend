...
-- Esclusivamente ambiente PROD. Distruttivo: rimuove solo le strutture P5.
...
BEGIN
    IF current_database() <> 'stima360_db' THEN
        RAISE EXCEPTION 'OWNER 0.2 P5 rollback bloccato: database corrente %, atteso stima360_db', current_database();
    END IF;
