...
#
# A proposal has no agency of its own. It names one match, and P26-4 certifies
# that a match joins a buy request and a property in the same agency. So the
# tenant is derived by re-reading that chain with the caller's scope bound to
# BOTH roots - filtering on one alone would still admit a pair whose other half
# belongs elsewhere.
#
# The legacy functions above are untouched: tests/test_next6_p1_proposals.py
# calls them directly. BUY and MATCH established the pattern - add `_scoped`
# rather than change a contract other callers depend on.
#
# The agency comes from ctx.require_agency() and from nowhere else. The actor
# still comes from require_admin, server-side, exactly as before.
# ---------------------------------------------------------------------------
...
def _scoped_relation(cur, match_id: int, agency_id: int, *, lock: bool = False):
    """The match, visible only when both of its roots are in the agency."""
    suffix = " FOR UPDATE OF m,b,p" if lock else ""
    cur.execute(
        f"""SELECT m.id AS match_id,m.archived_at AS match_archived_at,
            b.id AS buy_request_id,b.title AS buy_title,b.contact_id,b.lead_id,
            ...
            JOIN contacts c ON c.id=b.contact_id
            JOIN properties p ON p.id=m.property_id
            WHERE m.id=%s AND b.agency_id=%s AND p.agency_id=%s{suffix}""",
...
def _scoped_proposal(cur, proposal_id: int, agency_id: int, *, lock: bool = False):
    suffix = " FOR UPDATE OF pp" if lock else ""
    cur.execute(
        f"""SELECT pp.*,NOW() AS database_now,
            m.buy_request_id,m.property_id,
            ...
            c.display_name AS contact_name,
            p.title AS property_title,p.code AS property_code
            FROM property_proposals pp {_CHAIN_JOIN}
            WHERE pp.id=%s AND {_CHAIN_SCOPE}{suffix}""",
...
def create_proposal_scoped(ctx, data: dict, created_by: str):
    agency_id = _agency(ctx)
    data = dict(data)
    data["idempotency_key"] = str(data["idempotency_key"])
    with core_cursor(commit=True) as (_, cur):
        relation = _scoped_relation(cur, data["match_id"], agency_id, lock=True)

        # The idempotency lookup is scoped. The key is globally unique, so a
        # replay of another tenant's key must not return that tenant's record:
        # scoped, the lookup finds nothing and the insert below hits the unique
        # index, which surfaces as a conflict. The caller learns only that the
        # key is taken - unavoidable for a globally unique key, and not the row.
        cur.execute(
            f"""SELECT pp.* FROM property_proposals pp {_CHAIN_JOIN}
                WHERE pp.idempotency_key=%s AND {_CHAIN_SCOPE} FOR UPDATE OF pp""",
        ...
        if existing:
            if not _same_create_payload(existing, data):
                raise ConflictError("idempotency key already used with a different payload")
            return _scoped_proposal(cur, existing["id"], agency_id)

        # Deliberately not agency-scoped, and safe because of what precedes it:
        # `_scoped_relation` above already proved this match belongs to the
        # caller's agency, and a match belongs to exactly one - P26-4 enforces
        # that its buy request and property share a tenant. So every proposal on
        # this match is in this agency, and a predicate here would filter on a
        # condition that cannot fail. The uniqueness it protects is per-match,
        # not per-tenant.
        cur.execute(
            """SELECT id FROM property_proposals
            WHERE match_id=%s AND status IN ('draft','submitted') FOR UPDATE""",
        ...
        return _scoped_proposal(cur, created["id"], agency_id)


def get_proposal_scoped(ctx, proposal_id: int):
    agency_id = _agency(ctx)
    with core_cursor() as (_, cur):
        return _scoped_proposal(cur, proposal_id, agency_id)


def list_proposals_scoped(
    ctx,
    *,
    match_id: int | None = None,
    buy_request_id: int | None = None,
...
def update_proposal_scoped(ctx, proposal_id: int, data: dict, created_by: str):
    agency_id = _agency(ctx)
    data = dict(data)
    with core_cursor(commit=True) as (_, cur):
        snapshot = _scoped_proposal(cur, proposal_id, agency_id)
        _scoped_relation(cur, snapshot["match_id"], agency_id, lock=True)
        current = _scoped_proposal(cur, proposal_id, agency_id, lock=True)
        if current["status"] != "draft":
            raise ConflictError("only draft proposals can be updated")
        if not data:
            return current
...
def transition_proposal_scoped(ctx, proposal_id: int, target_status: str, created_by: str):
    agency_id = _agency(ctx)
    with core_cursor(commit=True) as (_, cur):
        snapshot = _scoped_proposal(cur, proposal_id, agency_id)
        _scoped_relation(cur, snapshot["match_id"], agency_id, lock=True)
        ...
        if target_status == "accepted":
            if expired:
                raise ValidationError("proposal is expired")
            cur.execute(
                "SELECT id FROM properties WHERE id=%s AND agency_id=%s FOR UPDATE",
                (current["property_id"], agency_id),
            )
            if not cur.fetchone():
                raise NotFoundError(f"property {current['property_id']} not found")
            # Scoped: another agency's accepted proposal must not make this
            # transition fail. An unscoped probe returns no row but still lets
            # one tenant's data cause a visible conflict in another.
            cur.execute(
                f"""SELECT pp.id FROM property_proposals pp {_CHAIN_JOIN}
                WHERE m.property_id=%s AND pp.status='accepted' AND pp.id<>%s
                  AND {_CHAIN_SCOPE}
                LIMIT 1""",
                (current["property_id"], proposal_id, agency_id, agency_id),
            )
            if cur.fetchone():
                raise ConflictError("accepted proposal already exists for property")
        ...
        return _scoped_proposal(cur, updated["id"], agency_id)
